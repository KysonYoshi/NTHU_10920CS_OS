...
// then, copy in the code and data segments into memory
   ...
   for (int i=0 ; i<PageSize; i++)
    ...
    //copy code to swap file
    for (int i=0 ; i<numPages; i++)
    {
        maptoSwap.insert(pair<int, int>(i, kernel->getSwapCtr()));
        kernel->incSwapCtr();
        executable->ReadAt(buff, PageSize, noffH.code.inFileAddr + (i * PageSize));
        kernel->swapFile->WriteAt(buff, PageSize, maptoSwap[i]*PageSize);
    }
...
void AddrSpace::setAttr(bool Valid, int phyPage, int virtPage)
{
    pageTable[virtPage].valid=Valid;
    pageTable[virtPage].physicalPage=phyPage;
    pageTable[virtPage].virtualPage=virtPage;
    //pageTable[virtPage].use= TRUE;
}


int AddrSpace::getVPN(int index)
{
    for (int i = 0; i < numPages; i++)
    {
        if(pageTable[i].physicalPage==index)
        {
            pageTable[i].valid=FALSE;
            pageTable[i].physicalPage= -1;
